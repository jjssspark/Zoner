...
| ID | 날짜 | 문제 | 상태 |
|---|---|---|---|
| TS-021 | 2026-08-13 | 데모 영상 편집에서 개인정보 구간을 목록에서 뺐는데도 결과물에 그대로 들어갔다. ffmpeg `-t` 를 `-i` 뒤에 두면 배속 필터 *이후* 출력 기준으로 잘려, 6배속 구간이 입력을 6배 읽어 제외 구간을 삼켰다. 에러도 경고도 없이 exit 0 | 해결 |
...
## TS-018 — 인쇄 배경: 검증 도구가 검증 대상을 우회했다

**날짜** 2026-08-09 · **상태** 해결

### 증상

리포트 PDF 저장 기능을 만들고 사용자가 실제로 뽑은 PDF에서, 콘텐츠가 끝난
지점부터 페이지 끝까지가 **검게** 인쇄됐다. 윗부분(제목·수치·그래프)은
정상적으로 흰 배경에 검은 글씨였다.

### 재현 조건

- `/read` 에서 `PDF 저장` → 인쇄 대화상자에서 **"배경 그래픽" 옵션을 켠 채** 저장
- 문서 길이가 한 페이지에 못 미칠 때 그 아래가 검게 나온다

### 원인

**표면**: `.session-report` 의 흰 배경이 자기 콘텐츠까지만 덮는다.

**근본**: `src/index.css` 가 `body { background-color: var(--color-bg) }` 인데,
`SessionReport.css` 의 인쇄 블록은 `--color-bg` 를 **`.session-report` 스코프
안에서만** 흰색으로 덮었다. `body` 는 `:root` 값(navy-950)을 그대로 썼다.

브라우저는 기본적으로 배경색을 인쇄하지 않으므로 평소엔 드러나지 않는다.
그런데 이 기능은 **막대 색을 데이터로 쓰기 때문에 "배경 그래픽"을 켜야 한다.**
켜는 순간 body 의 다크 배경도 함께 인쇄된다. 즉 **기능이 요구하는 설정이
버그를 활성화하는 구조**였다.

### 시도했지만 안 된 것 — 아니, 애초에 못 잡은 것

인쇄 대화상자는 모달이라 브라우저 자동화가 그 자리에서 멈춘다. 그래서
`@media print` 블록만 뽑아 화면용으로 벗겨낸 하니스를 만들어 검증했다.
세 번 검증했고 세 번 다 통과했다.

**그 하니스가 `body{width:680px;background:white}` 로 흰 배경을 강제하고
있었다.** A4 폭을 흉내내려고 넣은 줄에 `background:white` 를 딸려 넣은 것인데,
그게 정확히 이 버그의 증상을 지워버렸다. 게다가 하니스는 `tokens.css` 와
화면 CSS 만 로드하고 **`index.css`(전역)를 아예 태우지 않았다.**

검증 도구가 검증 대상을 우회하고 있었다.

### 해결

`src/index.css` 에 전역 인쇄 규칙을 넣었다.
...
하니스도 고쳤다 — `index.css` 를 파일 목록에 넣고, 강제하던
`background:white` 를 걷어냈다. 그 상태에서 다시 확인해 콘텐츠 아래가
흰색으로 나오는 것을 봤다.

### 검증

- 하니스(전역 CSS 포함, body 배경 강제 없음)에서 콘텐츠 아래 흰색 확인
- 빌드 성공, 테스트 268건 통과

**여전히 미검증**: 실제 인쇄 대화상자를 열지 못한다. 진짜 PDF 파일과
`print-color-adjust: exact` 가 실제로 먹는지는 사람이 확인해야 한다.

### 추후 관리

- 인쇄 스타일을 건드릴 때 **스코프를 반드시 확인한다.** 의미 토큰을 특정
  선택자 안에서 덮으면 그 바깥은 여전히 원래 값이다. `html`·`body` 처럼
  스코프 밖에 있는 것들을 놓치기 쉽다
- 하니스 CSS 목록은 실제 앱의 import 목록과 같아야 한다. 하나라도 빠지면
  그 파일에서 오는 버그는 영영 안 보인다

### 배운 점

**검증 환경이 실제와 다른 지점이 곧 사각지대다.** 이번엔 그 차이를 내가
직접 만들었다 — 폭을 맞추려고 넣은 한 줄에 배경색이 딸려 들어갔고, 그게
하필 검증하려던 그 속성이었다.

TS-017 과 묶어 보면 패턴이 같다. **"애니메이션이 재생되지 않는 환경",
"배경 그래픽이 켜진 인쇄" 처럼 기본값이 아닌 조건에서만 드러나는 버그를,
기본값만 재현하는 도구로 찾으려 했다.** 검증 도구를 만들 때는 무엇을
재현하는지보다 **무엇을 재현하지 않는지**를 먼저 적어두는 편이 낫다.

---

## TS-019 — 인쇄 토큰 오버라이드가 CSS 소스 순서에 졌다

**날짜** 2026-08-10 · **상태** 해결

### 증상

HUD 재설계에서 인쇄 시 노치(`clip-path`)·글로우·계측 격자를 한 번에 벗기려고
`@media print` 안에서 토큰 자체를 덮었다. 태스크 리뷰도, 가드 테스트도,
전체 테스트도 전부 통과했다. **그런데 인쇄에서 아무것도 벗겨지지 않았다.**

리포트 차트는 인쇄용으로 `overflow-x: visible` 을 줘서 전체가 나오게 해뒀는데,
살아남은 `clip-path` 가 요소 박스 기준으로 다시 잘라 **오른쪽 막대가 잘려나갔다.**

### 재현 조건

- `/read` 에서 인쇄 미리보기 (배경 그래픽 옵션과 무관하게 재현)
- 프로덕션 빌드에서만 확인 가능 — 개발 서버는 CSS를 파일별로 주입해서
  순서가 다르게 보일 수 있다

### 원인

**표면**: 인쇄 블록의 `:root` 오버라이드가 적용되지 않는다.

**근본**: `@media` 는 명시도를 올리지 않는다. 아래 둘은 **명시도가 같다.**

```css
/* src/index.css */
@media print { :root { --notch-path: none; } }

/* src/styles/tokens.css */
:root { --notch-path: polygon(...); }
```

명시도가 같으면 **소스 순서가 이긴다.** 그리고 이 앱의 import 순서는

```
src/index.js:3   import './index.css';     ← 먼저 실린다
src/App.js:4     import './styles/tokens.css';  ← 나중에 실린다
```

이라서 `tokens.css` 의 기본값이 항상 인쇄 오버라이드를 덮었다. 빌드 산출물에서
확인한 바이트 오프셋이 그대로 보여준다 — 인쇄 `:root{` 841, 기본 `:root{` 1044.

"인쇄 블록이니까 인쇄에서 이긴다"는 직관이 틀렸다. 미디어 쿼리는 **조건**이지
**우선순위**가 아니다.

### 시도했지만 안 된 것 — 아니, 못 잡은 것

- 태스크 3 리뷰가 이 지점을 정확히 지목해 검사했다. 리뷰어는
  "두 곳에서만 정의되고, 인쇄 `:root` 가 나중 규칙이라 캐스케이드로 이긴다"고
  결론냈다. **파일 안에서의 순서만 보고 파일 사이의 순서를 안 봤다.**
- 가드 테스트(`tokens.test.js`)는 `index.css` 텍스트에 `--notch-path: none` 이
  들어 있는지만 확인했다. **텍스트는 있고 효력은 없는 상태**를 구분하지 못한다.
- 전체 테스트 271건, 프로덕션 빌드 전부 통과. jsdom 은 캐스케이드를 평가하지 않는다.

TS-018 과 같은 계열이다. 그때는 검증 하니스가 대상을 우회했고, 이번엔
검증 대상이 **소스 텍스트**여서 런타임 동작을 못 봤다.

### 해결

인쇄 `:root` 오버라이드를 `index.css` 에서 빼서 **`tokens.css` 맨 끝**으로 옮겼다.
같은 파일 안에서 기본 선언보다 뒤에 오므로 순서로 이긴다.

```css
/* src/styles/tokens.css 맨 끝 */
@media print {
  :root {
    --notch-path: none;
    --notch-path-sm: none;
...
`index.css` 의 나머지 인쇄 규칙(`html, body` 흰 배경, `.hud-brackets` 숨김)은
그대로 뒀다 — 다른 속성/선택자라 이 문제의 영향을 받지 않는다.

### 검증

프로덕션 빌드 산출물의 바이트 오프셋으로 직접 확인했다.

```
수정 전: 인쇄 :root{ @841   <  기본 :root{ @1044   (기본이 이김 — 버그)
수정 후: 기본 :root{ @961   <  인쇄 :root{ @4710   (인쇄가 이김)
```

가드 테스트도 **존재 여부가 아니라 순서**를 단언하도록 바꿨다. 블록을 파일
앞쪽으로 되돌리면 실패하는 것을 실제로 확인했다(`Expected: > 2521, Received: 2521`).

### 추후 관리

- **`@media` 안에서 `:root` 커스텀 프로퍼티를 덮을 때는 정의부와 같은 파일,
  같은 스코프, 더 뒤에 둔다.** 파일이 다르면 import 순서에 지배당한다.
- CSS 검증은 소스 텍스트가 아니라 **빌드 산출물의 순서**를 봐야 잡힌다.
  텍스트 매칭 테스트는 "썼는지"만 알려주고 "먹히는지"는 모른다.
- 인쇄 경로는 자동 검증이 닿지 않는다. 리포트를 건드리면
  **프로덕션 빌드로** 인쇄 미리보기를 눈으로 확인한다.

### 배운 점

리뷰 세 겹(구현자 자체 리뷰 → 태스크 리뷰 → 최종 브랜치 리뷰)을 통과하고
...
그리고 배포는 그 자체가 **여태 없던 검증 계층**이었다. 고도화를 시작한
2026-08-04부터 8일치 코드가 로컬 검증을 전부 통과했지만, 처음 배포하자마자
즉시 깨졌다.
검증 환경과 실행 환경의 거리가 멀수록 통과의 의미는 약해진다.

---

## TS-021 — 잘라낸 줄 알았던 구간이 편집본에 그대로 들어가 있었다

**날짜** 2026-08-13 · **상태** 해결

### 증상

포트폴리오 데모 영상을 편집했다. 3분 10초 원본을 18개 구간으로 나눠
구간마다 다른 배속을 걸고 이어 붙이는 방식이다. 개인정보가 찍힌 3개 구간
(Discord 창, 확대된 Dock, macOS 창 정렬 메뉴)은 구간 목록에서 아예 뺐다.

렌더링은 **에러 없이 성공**했다. 그런데 결과 길이가 이상했다.

```
[18/18] outro  170.0s +4.0s  2.0x
132.800000
```

구간별 계산상 나와야 할 길이는 58초인데 **132.8초**가 나왔다.

### 재현 조건

배속 필터(`setpts`)를 쓰는 모든 구간에서 항상 재현된다. 등속(1.0x) 구간만
있으면 드러나지 않는다. 배속이 클수록 오차가 커진다.

```bash
# 문제가 있는 형태
ffmpeg -ss 13.5 -i src.mov -t 16.5 -filter_complex "[0:v]setpts=PTS/6.0[v]" -map "[v]" out.mp4
```

### 원인

**표면**: 출력 길이가 계산값의 2배 이상.

**근본**: `-t` 를 `-i` **뒤에** 두면 입력이 아니라 **출력** 타임라인 기준으로
자른다. 필터그래프가 먼저 돌기 때문에, 6배속 구간은 "출력 16.5초"를 채우려고
**입력을 99초 읽는다.**

```
의도:  입력 13.5s ~ 30.0s (16.5초) → 6배속 → 출력 2.75초
실제:  입력 13.5s ~ 112.5s (99초)  → 6배속 → 출력 16.5초
```

문제는 길이가 아니다. **잘라내려고 목록에서 뺐던 43.5~47.5초 Discord 구간이
그 99초 안에 통째로 들어 있었다.** 제외 구간을 명시적으로 건너뛰는 설계였는데,
읽기 범위가 조용히 넘쳐서 제외가 무의미해졌다.

에러도 경고도 없다. 사람이 최종 결과물을 프레임 단위로 보지 않으면
**개인정보가 들어간 채로 public 저장소에 커밋된다.**

### 시도했지만 안 된 것 — 정확히는, 하마터면 안 잡을 뻔한 것

실패한 수정 시도는 없었다. 길이 불일치를 보고 바로 원인을 찾았다.

위험했던 건 **길이를 확인하지 않았을 경우**다. 렌더링은 exit 0이었고 구간별
진행 로그도 18개 전부 정상 출력됐다. `ffprobe` 로 길이를 찍어보는 한 줄이
없었다면 그대로 넘어갔다.

### 해결

`-t` 를 `-i` 앞으로 옮겨 **입력 읽기 범위**를 제한한다.

```bash
# -t 는 -i 앞에 둔다. 뒤에 두면 배속 후 출력 기준으로 잘려 입력을 배속배만큼 더 읽는다.
ffmpeg -ss 13.5 -t 16.5 -i src.mov -filter_complex "[0:v]setpts=PTS/6.0[v]" -map "[v]" out.mp4
```

### 검증

1. 길이: 132.8초 → **58.2초**. 구간별 계산값과 일치한다.
2. 완성본을 1초 간격 프레임으로 전부 추출해 58장을 눈으로 확인했다.
   Discord·Dock·창 정렬 메뉴 모두 없다.

이 프레임 전수 검사에서 **다른 두 건이 추가로 잡혔다.**

- 영상 저장 확인 대화상자를 잡았다고 생각한 구간(95~99.5초)이 실제로는
  대화상자가 뜨기 **직전**이었다. 대화상자는 99.3초에 뜬다. 핵심 기능
  장면인데 편집본에 한 프레임도 안 들어가 있었다.
- 리포트 진입 직전에 빈 흰 화면이 2초 남아 있었다.

### 추후 관리

영상 편집 스크립트에는 **길이 어서션**을 넣는다. 구간 목록에서 기대 길이를
계산해 실제 출력과 비교하고, 오차가 크면 실패시킨다. 이번엔 사람이 숫자를
읽어서 잡았지만 그건 재현되는 검증이 아니다.

제외 구간이 있는 편집은 **완성본 프레임 전수 검사를 생략하지 않는다.**
어제(2026-08-11) 작업에서도 이 검사에서만 잡힌 누락이 두 건 있었다.

### 배운 점

**옵션의 위치가 의미를 바꾸는데 에러로 알려주지 않는 도구가 있다.**
`-t` 는 `-i` 앞뒤 어디에 써도 문법 오류가 아니다. 앞이면 입력 트림,
뒤면 출력 트림이고, 필터가 시간축을 바꾸면 둘은 전혀 다른 결과를 낸다.

**"목록에서 뺐으니 안 들어간다"는 보장이 아니다.** 제외는 읽기 범위가
의도대로일 때만 성립하는 조건부 보장이었다. 안전장치를 설계할 때
그 장치가 기대는 전제가 뭔지를 같이 봐야 한다.

TS-018·TS-020과 같은 계열이다. 검증이 통과했는데 실제로는 검증이
문제를 볼 수 없는 조건이었다. 이번엔 "exit 0"이 그 역할을 했다.
